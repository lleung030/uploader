...
const generateCanvas = (file: any, fileType: string, divisionBy = 5, quality = 0.75) => {
  return new Promise((resolve, reject) => {
    ...
    if (fileType === 'image') {
      ...
      audio.onloadeddata = () => {
        // canvas.width = audio.audioWidth / divisionBy;
        // canvas.height = audio.audioHeight / divisionBy;
        // context.drawImage(audio, 0, 0, 
          // canvas.width, canvas.height
          // );
        audio.pause();
        return context.canvas.toBlob((blob: any) => resolve(blob), 'audio/mpeg', quality);
      };

      audio.onerror = () => reject(`${file.name} is invalid video format.`);
